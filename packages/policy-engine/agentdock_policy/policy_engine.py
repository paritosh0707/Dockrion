...
class PolicyEngine:
    def __init__(self, tools_allowed: list[str], deny_by_default: bool, redact_patterns: list[str], max_out: int|None):
    ...
    def from_dockspec(cls, spec):
        ...
        deny = (spec.policies.tools.deny_by_default if spec.policies and spec.policies.tools else True)
        red  = (spec.policies.safety.redact_patterns if spec.policies and spec.policies.safety else [])
        mlen = (spec.policies.safety.max_output_chars if spec.policies and spec.policies.safety else None)
